...
const Player = (n, m) => {
  ...
  return { getMark, getName, changeName };
};
...
const displayControl = (() => {
  ...
  const result = () => {
    ...
    if (winner == "Draw") {
      ...
      if (winner == "Player 1") player1.classList.remove("current");
      else player2.classList.remove("current");
    } else {
  ...
  function changeMode(m) {
    ...
    blocks.forEach((b) => (b.textContent = ""));
    let currentPlayer = Game.getCurrentPlayer().getName();
